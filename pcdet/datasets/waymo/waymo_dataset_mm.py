...
class WaymoDatasetMM(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.cam_types = dataset_cfg.get('CAMRA_TYPES', None)
        if self.cam_types is None:
            self.cam_types = CAMERA_TYPES
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            #     info_path = self.data_path / sequence_name / ('%s_fov_only.pkl' % sequence_name)
    ...
    def __getitem__(self, index):
        ...
        images_index = []
        calibs = []
        for i, cam_key in enumerate(self.cam_types):
            calib = self.kitti_calib_convert(images_info[cam_key])
            img_path = self.data_path / sequence_name / images_info[cam_key]['img_path']
            images.append(self.get_image(img_path))
            images_path.append(img_path)
            images_index.append(images_info[cam_key]['img_path'])
            images_shape.append((images_info[cam_key]['height'], images_info[cam_key]['width']))
            calibs.append(calib)
        ...
        if self.dataset_cfg.FOV_POINTS_ONLY:
            ...
            img_shape = images_shape[0]
            calib_fov = calibs[0]
            pts_rect = calib_fov.lidar_to_rect(points[:, 0:3])
            fov_flag = self.get_fov_flag(pts_rect, img_shape, calib_fov)
            points_fov = points[fov_flag]
            image_fov = images[0]
            #Filter gt_bboxes outside FOV
            gt_boxes_center_3d = gt_boxes_lidar[:, 0:3]
            gt_boxes_center_rect = calib_fov.lidar_to_rect(gt_boxes_center_3d)
            gt_boxes_fov_flag = self.get_fov_flag(gt_boxes_center_rect, img_shape, calib_fov)
            gt_boxes_fov = gt_boxes_lidar[gt_boxes_fov_flag]
            gt_names_fov = annos['name'][gt_boxes_fov_flag]
            num_points_in_gt = input_dict['num_points_in_gt']
            if num_points_in_gt is not None:
                num_points_in_gt_fov = num_points_in_gt[gt_boxes_fov_flag]
            input_dict.update({
                'points': points_fov,
                'images': image_fov,
                'calib': calib_fov,
                'gt_names': gt_names_fov,
                'gt_boxes': gt_boxes_fov,
                'num_points_in_gt': num_points_in_gt_fov
                })
        
        if "calib_matricies" in get_item_list:
            trans_lidar_to_cam = []
            trans_cam_to_img = []
            for calib in input_dict["calibs"]:
                trans_lidar_to_cam_i, trans_cam_to_img_i = kitti_utils.calib_to_matricies(calib)
                trans_lidar_to_cam.append(trans_lidar_to_cam_i)
                trans_cam_to_img.append(trans_cam_to_img_i)
            input_dict["trans_lidar_to_cam"] = trans_lidar_to_cam 
            input_dict["trans_cam_to_img"] = trans_cam_to_img
            
        data_dict = self.prepare_data(data_dict=input_dict)
        data_dict['metadata'] = info.get('metadata', info['frame_id'])
        data_dict.pop('num_points_in_gt', None)
        if "image_paths" in get_item_list:
            data_dict.update({
    ...
    def kitti_calib_convert(self, calib):
        #Convert waymo calib to CalibrationKitti
        P2 = np.array(torch.Tensor(calib.get('cam2img')))
        R0 = np.eye(3).astype(np.float32)
        V2C = calib.get('lidar2cam')
        V2C = np.array(torch.Tensor(V2C))[:3, :]
        kitti_calib = {
            'P2': P2,
            'R0': R0,
            'Tr_velo2cam': V2C
        }
        return calibration_kitti.Calibration(kitti_calib)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        if self.dataset_cfg.FOV_POINTS_ONLY:
            for idx, info in enumerate(self.infos):
                fov_image_info = info['images']
                calib_fov = self.kitti_calib_convert(fov_image_info['CAM_FRONT'])
                fov_images_shape = (fov_image_info['CAM_FRONT']['height'], 
                                         fov_image_info['CAM_FRONT']['width'])
                #Filter gt_bboxes outside FOV
                gt_boxes_lidar = eval_gt_annos[idx]['gt_boxes_lidar']
                gt_boxes_center_3d = gt_boxes_lidar[:, 0:3]
                gt_boxes_center_rect = calib_fov.lidar_to_rect(gt_boxes_center_3d)
                gt_boxes_fov_flag = self.get_fov_flag(gt_boxes_center_rect, fov_images_shape, calib_fov)
                gt_boxes_fov = gt_boxes_lidar[gt_boxes_fov_flag]
                gt_names_fov = eval_gt_annos[idx]['name'][gt_boxes_fov_flag]
                num_points_in_gt = eval_gt_annos[idx]['num_points_in_gt']
                if num_points_in_gt is not None:
                    num_points_in_gt_fov = num_points_in_gt[gt_boxes_fov_flag]
                eval_gt_annos[idx].update({
                    'name': gt_names_fov,
                    'difficulty': eval_gt_annos[idx]['difficulty'][gt_boxes_fov_flag],
                    'dimensions': eval_gt_annos[idx]['dimensions'][gt_boxes_fov_flag],
                    'location': eval_gt_annos[idx]['location'][gt_boxes_fov_flag],
                    'heading_angles': eval_gt_annos[idx]['heading_angles'][gt_boxes_fov_flag],
                    'obj_ids': eval_gt_annos[idx]['obj_ids'][gt_boxes_fov_flag],
                    'tracking_difficulty': eval_gt_annos[idx]['tracking_difficulty'][gt_boxes_fov_flag],
                    'num_points_in_gt': num_points_in_gt_fov,
                    'speed_global': eval_gt_annos[idx]['speed_global'][gt_boxes_fov_flag],
                    'accel_global': eval_gt_annos[idx]['accel_global'][gt_boxes_fov_flag],
                    'gt_boxes_lidar': gt_boxes_fov,
                    })
