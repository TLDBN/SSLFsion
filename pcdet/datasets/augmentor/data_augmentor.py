...
class DataAugmentor(object):
    ...
    def imgaug(self, data_dict=None, config=None):
        ...
        #data_dict["images"] = new_imgs
        return data_dict

    def img_normalize(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.img_normalize, config=config)
        img = data_dict["images"].copy().astype(np.float32)
