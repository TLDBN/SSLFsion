...
def apply_3d_transformation(pcd, coord_type, img_meta, reverse=False):
    ...
    #     #img_meta['pcd_rotation'].clone().detach().requires_grad_(True)
    ...
    pcd_rotate_mat =(torch.tensor(lidar_aug_matrix[..., :3, :3], dtype=dtype, device=device))

    pcd_scale_factor = batch_dict['nosie_scale']

    pcd_trans_factor = lidar_aug_matrix[..., :3, 3]
