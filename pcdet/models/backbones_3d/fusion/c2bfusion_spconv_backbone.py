...
class C2BFusion(nn.Module):
    def __init__(self,
        ...
        if dataset=='kitti':
            ...
            self.point_cloud_range = torch.Tensor(point_cloud_range).cuda().squeeze_()
            self.voxel_size = torch.Tensor(voxel_size).cuda()
        elif dataset=='nus':
            point_cloud_range = [ -5.0, -51.2, -51.2, 3.0, 51.2, 51.2]
            voxel_size = [0.2, 0.1, 0.1]
            self.inv_idx = torch.Tensor([2, 1, 0]).long().cuda()
            self.point_cloud_range = torch.Tensor(point_cloud_range).cuda().squeeze_()
            self.voxel_size = torch.Tensor(voxel_size).cuda()
        ...
        self.voxel_transform = nn.Sequential(
            nn.Linear(voxel_channel, out_channel),
    ...
    def forward(self, sp_voxel, batch_dict):
        """Forward function.
        ...
        spatial_indices = sp_voxel.indices[:, 1:]
    ...
    def point_sample_nus(self, batch_dict, voxels_feats, img_feats, voxels_3d, batch_index):
        ...
        batch_size = batch_dict['batch_size']
        for level in range(len(self.img_levels)):
            BN, C, H, W = img_feats[level].size()
            img_feats[level] = img_feats[level].view(int(BN / 6), 6, C, H, W)
        ...
        for i in range(batch_size):
            ...
            for level in range(len(self.img_levels)):
                img_inter_in = []
                for j in range(6):
                    single_level_inter_feats = F.adaptive_avg_pool2d(img_feats[level][i][j:j + 1], (1, 1))
                    img_inter_in.append(single_level_inter_feats.squeeze_().unsqueeze_(0))
                img_inter_in = torch.cat(img_inter_in)
                img_inter.append(img_inter_in)
            img_inter = torch.cat(img_inter, dim=-1)
            layer_weight = self.LayerAttention(img_inter)
            for level in range(len(self.img_levels)):
                for j in range(6):
                    img_feats[level][i][j:j + 1] = img_feats[level][i][j:j + 1].clone() * layer_weight[j, level] + img_feats[level][i][j:j + 1].clone()
        img_feats = torch.stack(img_feats, dim=1)
        img_feat = img_feats.view(batch_size, 6, -1, H, W)
        ...
        for b in range(batch_size):
            ...
            if self.training and self.dropout_ratio > 0:
                img_pts = F.dropout((img_pts), p=self.dropout_ratio)
            img_feats_pre.append(img_pts)
    ...
    def point_sample_single_kitti(self, batch_dict, batch_index, voxels_feats, img_feats, voxels_3d, calib):
        ...
        image_pts = F.dropout(image_pts, p=0.1)
